...
## UI manual flow + in-app log + EMPTY Инфо/Изоҳ columns (2026-07-10)
Two changes to the C#/Avalonia desktop app in `multicard/` (all built + unit-tested on the dev Mac; `dotnet build` clean, **10/10 Core.Tests pass**; live fetch still geo-blocked from the non-UZ dev IP so the fetch→write path is unit-tested with fakes, not driven against the live API):

**1. Row-mapping DECISION — Инфо (D) and Изоҳ (J) are now written EMPTY** for BOTH credit and debit (user/accountant request 2026-07-10, shown via two sheet screenshots). This **supersedes** the "Row mapping (build_row)" section above: the `info` col-D label (`MultiCard пополнение`/`списание`) and the `izoh` col-J text (datetime + long deposit note like "…ПОПОЛНЕНИЕ ОБЕСПЕЧИТЕЛЬНОГО ДЕПОЗИТА…") must NOT land in the sheet. Still written: Сана (A), Приход/Расход (F/H), and Тўлов тури (G/I = `Мультисард`). Implemented in `Core/Logic/RowBuilder.cs` → `[date,"","","","",prixod,tolovP,rasxod,tolovR,""]`; `RowBuilderTests` updated to assert cols 3 and 9 are `""`.

**2. New manual UI flow (two-step) + live log panel** — the auto sync loop is unchanged; these are additive:
- **⬇️ Tortib olish** → `SyncService.PreviewAsync(ct)`: fetches the current adaptive-lookback window and returns `PreviewTransaction(Key,Type,Tx,IsNew)` — **does NOT write, does NOT mark seen**. `IsNew = !seen.Contains(key)`. Shown in a new "Tortib olingan" tab (🆕 Yangi / • Sinxronlangan).
- **📤 Sheetga yozish** → `SyncService.WritePreviewAsync(preview,ct)`: writes ONLY the `IsNew` rows, marks them seen, heartbeat + lastSync. **Baseline-guarded**: if `IsFirstRunAsync` (no lastSync yet) it marks baseline and writes nothing — same safety as `RunOnceAsync`, so a fresh DB can't dump full history. Both new methods take the same `_gate` SemaphoreSlim → serialized with the background loop.
- **📜 Log tab** → `App/Services/UiLogSink.cs`: a singleton Serilog `ILogEventSink` (`UiLogSink.Instance`, registered in `Program.cs` beside the file sink) raises an event per log line; `MainWindowViewModel` subscribes and pushes to an `ObservableCollection<LogEntryViewModel>` (newest-first, capped 500, marshalled to UI thread). Because `SyncService.Log` is already routed to `ILogger`, all sync + app logs appear live in-app (same content as the `%APPDATA%\MultiCardSync\logs` file).
- `MainWindow.axaml` is now a `TabControl`: **Sinxron tarixi** (old DataGrid) / **Tortib olingan** (preview) / **Log**. New VM members: `PreviewRows`, `Logs`, `IsBusy`/`NotBusy`/`CanWrite`/`HasPreview`, `PreviewSummary`, `FetchDataCommand`, `WriteToSheetCommand`. New Core model `PreviewTransaction`; `ToItem` refactored to `(key,type,tx,status,now)`.

STATUS: code written + built + tested locally; NOT yet committed. Per the earlier push gotcha (fly-deploy.yml triggers on any push to main), commit on a branch + PR to build the `.exe` via `multicard-build.yml` without redeploying the Fly Telegram bot. See [[park-api-credentials]].


### Revision same day (2026-07-10): per-ROW write button (supersedes the bulk "📤 Sheetga yozish")
User refined the write UX: instead of ONE bulk button that writes all new rows, **each preview row has its own 📤 Yozish button** that writes only THAT record. Implementation:
- `SyncService.WritePreviewAsync(list)` was **removed**; replaced by `WriteOneAsync(PreviewTransaction p, ct) → bool`: idempotent (skips if `seen.Contains(p.Key)`), appends the single row, marks it seen (Written/Queued), fires `ItemsSynced`, best-effort heartbeat.
- **Baseline-safety invariant (key):** `WriteOneAsync` advances `lastSync` ONLY if it is already non-null. On a fresh DB (firstRun, `lastSync==null`) it writes the chosen row but leaves `lastSync==null`, so the background `RunOnceAsync` stays in first-run mode and BASELINES the remaining window history (marks seen, no write) instead of dumping it to the sheet. The just-written row is already `seen` so it's excluded from that baseline and keeps its Written status. Unit-tested (`WriteOneAsync_on_first_run_writes_but_keeps_baseline_pending`).
- `PreviewRowViewModel` is now an `ObservableObject` holding its `PreviewTransaction` + a `Func<PreviewRowViewModel,Task>` write callback; observable `IsWritten`/`IsBusy`; computed `CanWrite` (=`IsNew && !IsWritten && !IsBusy`) gating a `[RelayCommand(CanExecute=nameof(CanWrite))] WriteRow`; dynamic `State` (🆕 Yangi → ⏳ Yozilmoqda… → ✅ Yozildi; • Sinxronlangan for non-new). The parent VM's `WriteRowAsync` does the write + `LoadHistoryAsync`.
- `MainWindow.axaml` "Tortib olingan" tab gained a `DataGridTemplateColumn` "Amal" with the 📤 button (`{Binding WriteRowCommand}`, x:CompileBindings=False). Removed: the top bulk write button + VM `WriteToSheetCommand`/`HasPreview`/`CanWrite`/`_preview`.
- Tests: 11/11 pass, build clean. NOT driven live: the dev Mac is on a UZ IP so launching the app would hit the REAL MultiCard API and could WRITE to the live Sheet — not done during verification (outward-facing); logic covered by fakes instead.

## ⛔ TARGET SHEET BROKEN (2026-07-31) — see [[sheet-template-2026-07]]
The accountant re-templated the workbook ~2026-07-28: `Нахд Приход&Расход` moved to worksheet **index 0**, so this app's `WorksheetIndex = 1` resolved to `Инфо` and it appended MultiCard rows into that reference sheet (`Инфо`!I216:R232) — still arriving as of 2026-07-31 13:45. The A→J column map below is stale (data table is now **B..M starting at row 21**).

## Upgrading the `.exe` on the accountant's PC (procedure + one trap)
Runtime state lives in `%APPDATA%\MultiCardSync\` (`multicardsync.db` + `logs\`), **not** beside the exe — so swapping the binary preserves the dedup history. That matters: a fresh DB would make the app treat the run as first-run and baseline everything instead of writing.

**The trap:** `MainWindowViewModel.InitializeAsync` re-registers startup only when no entry exists (`… && !_startup.IsRegistered()`), so it never rewrites a stale path. Dropping the new build in a *different* folder leaves `HKCU\…\Run\MultiCardSync` pointing at the OLD exe — manual launch runs the new one, boot runs the old one, and you get two writers again. Replace **in place, same filename** (`MultiCardSync.exe`); if the path must change, toggle the app's startup checkbox off and on to rewrite the registry value. Find the current path with `reg query "HKCU\Software\Microsoft\Windows\CurrentVersion\Run" /v MultiCardSync`.

Order: quit via tray (Windows locks a running exe) → replace → Properties → *Unblock* (downloaded file) → run → SmartScreen "run anyway" (unsigned) → confirm the two log lines in the 📜 Log tab. Artifact `multicardsync-win-x64` (~43 MB) expires 30 days after each build.

**Code fixed 2026-07-31:** the app resolves the sheet by **`GoogleSheet:WorksheetTitle`** and anchors the append range to the "Сана" header row (`B20:K`) via `Core/Logic/TableAnchor`; `WorksheetIndex` remains only as a warned legacy fallback. **Not yet done:** the 17 junk rows in `Инфо` are still there, and the accountant's PC still runs a pre-`d074f37` `.exe` — stop it and re-distribute a new build. Full detail in [[sheet-template-2026-07]].

**UI/mapping changes by user (2026-07-10):** the desktop app gained a MANUAL flow alongside the auto loop — "⬇️ Tortib olish" (Preview: fetch current tx WITHOUT writing; SyncService.PreviewAsync), a per-row "📤 Yozish" button that writes ONE tx (SyncService.WriteOneAsync, idempotent + baseline-safe: writes but doesn't advance lastSync while baseline unset), a "📜 Log" tab (live Serilog via UiLogSink), and tabs (Sinxron tarixi / Tortib olingan / Log). MAPPING DECISION (supersedes the "Row mapping (build_row)" section above): per the accountant, `RowBuilder` now leaves **Инфо (col D) and Изоҳ (col J) EMPTY** for both credit & debit — no "MultiCard пополнение/списание" label, no long note text. Only date, direction amount (Приход/Расход F/H), and pay method "Мультисард" (G/I) are written. Tests updated accordingly. STATUS: user took over code edits; secrets set; push (branch+PR recommended) still pending the user.